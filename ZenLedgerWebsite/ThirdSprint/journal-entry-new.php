...
function AddNewSubentry(is_crediting, is_first)
            ...
            function AddNewSubentry(is_crediting, is_first) {
                ...
                new_input.type = "text";
                new_input.setAttribute("list", "account_names");
                new_input.placeholder = "Start typing category...";
    ...
    <p class="cosmic-message"><?php echo $cosmic_message; ?></p>
            ...
            <select id="account_name" name="account_name">
                <?php
                    ...
                    $query = "SELECT account_name FROM chart_of_accounts;";
                    $result = pg_query($dbconn, $query) or die('Query failed: ' . pg_last_error());
                                ...
                                if(total_debit == total_credit) {
                                    ...
                                    if(!hasDuplicates(debit_number_list)) {
                                        if(!hasDuplicates(credit_number_list))
                                        {
                                            ...
                                            if(intersection.length == 0)
                                            {
                                                ...
                                                async function send() {
                                                    try {
                                                       ...
                                                       <?php } else { ?>
                                                           window.location.replace("journal-entry-new.php");
                                                       <?php } ?>
                                                    }
                                            ...
                                            {
                                                error.innerHTML = "ERROR: Credit and Debit have subentries that share an account.\n";
                                            }
                                        }
                                        else {
                                            error.innerHTML = "ERROR: Credit side has subentries with duplicate accounts.";
                                        }
                                    }
                                    else {
                                        error.innerHTML = "ERROR: Debit side has subentries with duplicate accounts.";
                                    }
                                }
                                else {
                                    console.log("t_deb: "+total_debit+"\n");
                                    console.log("t_cre: "+total_credit+"\n");
                                    error.innerHTML = "ERROR: The total debit and total credit do not match.";
                                }
                            }
                            else {
                                error.innerHTML = "ERROR: Credit side has at least one subentry with a zero dollar amount.";
                            }
                        }
                        else {
                            error.innerHTML = "ERROR: Debit side has at least one subentry with a zero dollar amount.";
                        }
                    });
                </script>
            </div>
    ...
    <option value="Prepaid Rent">
    <option value="Prepaid Insurance">
    ...
    <option value="Salaries Payable">
    <option value="Interest Payable">
    <option value="Debts Payable">
